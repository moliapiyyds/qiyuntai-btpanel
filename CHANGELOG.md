...
### 交付路线改成「一镜像到底」：默认不再走宝塔官方源（2026-09-22 定）

起因是把面板卸了、用一键脚本重装一次（实测）。装是装成了，但**装到的是 13.1.0**：
...
而我们的补丁是**版本门禁**的（`tools/moli_patch.py` 的 `PANEL_VERIFIED = ['13.0.0']`），
于是当天走官方源的一键部署会在 `patch` 步骤**明确失败**，要人工核验后加 `--force` 才能放行。

**一个必须说准的结论**：加 `--force` 实测，13.1.0 上**锚点漂移是 0/8** ——
8 项校验全正常（永久企业版 / no-store / 免绑定视图 / 数据层 / 账户接口 / 去除更新 /
前端兜底 4 文件 / 浏览器版本门）、4 个 py 文件语法 OK、254 个 JS 版本戳都更新了。
所以问题**不是「补丁会坏」**，而是「面板版本不在我们手里 → 交付物不可复现 →
宝塔每发一版我们都得人工重核验一遍」。镜像解决的是这个。

改了什么：

* `install/deploy.sh` **默认走预制镜像**：先看 `--from-image` 指的目录，再看
  `/data/local/tmp/qyt-image`、`/data/qyt_image`、`/sdcard/qyt-image` 等默认位置，
  都没有就调 `fetch-image.sh` 自己下。从源铺 rootfs + 官方安装器那条退到 **`--from-source`**，
  并在运行时大声标明「面板版本随宝塔浮动、补丁可能拒绝执行，仅作者重建环境用」。
* 新增 **`install/fetch-image.sh`**：取镜像分卷。断点续传（`curl -C -` / `wget -c`）、
  逐卷 + 整包 sha256 校验、重试；`--tries 0` = 一直试到成功；`--from <目录>` 用本地已有的；
  `-u/--image-url` 换源；`--only` 单卷重取；`--check` 只校验。
  **失败时保留半截文件**（那正是断点续传要用的东西）。
* 新增 **`install/image.lock`**：镜像哈希清单（分卷 + 整包，带 tag）。
  `fetch-image.sh` 与 `deploy.sh` 都按它核对；`deploy.sh` 的校验顺序是
  `--image-sha` > 镜像目录里的 `SHA256SUMS.txt` > `install/image.lock`。
* `deploy.ps1` / `deploy-linux.sh` **把镜像也一起准备好**：电脑侧下到 `_dist/image/`
  （已 gitignore，校验通过不重复下）→ `adb push` 到手机 → 在手机上再核一遍 sha256 才算完。
  新增参数 `-ImageDir` / `-ImageUrl`（PS1）与 `-i/--image-dir` / `--image-url`（sh）。
  手机上已经有且校验通过就整段跳过。
* `tools/ci.sh` 新增第 **7b** 项：`install/image.lock` 的字段格式
  （类型只允许 `image`/`whole`、sha256 必须是 64 位小写 hex、字节数必须是纯数字、
  有分卷就必须有整包行 —— 缺了就说明解包前的整包校验会退化成「跳过」）。

### 修 `prepare-rootfs.sh --clean`：惰性卸载之后删不干净（端到端实测抓到）
...
`unmount_all` 用的是 `umount -l`（惰性卸载）：挂载立刻从命名空间摘掉，
但**挂载点目录本身**要等引用释放完才能删，紧接着 `rm -rf` 就会留下空的 `dev`。
安全设计本身是有效的（宿主 `/dev` 完好：256 项、`/dev/null` 仍是字符设备 1,3、
`/dev/socket` 29 项、`getprop` 正常），`deploy.sh` 也按设计拒绝了在非空目录上解包。
问题在于「残留一个空目录就判失败」太严 —— 现在改成**重试 6 次**（每次顺带 `rmdir` 空挂载点目录、
间隔 1 秒），仍然残留才失败，并提示「重启一次手机再来」。

### 文档口径统一

* `README.md`：首页入口改成「镜像为主」（电脑侧一键 → 纯手机 → 从已装好的设备直接拿），
  新增「为什么不走宝塔官方源」与「【作者用】从源重建环境（`--from-source`）」两节；
  「仓库结构」补上 `install/fetch-image.sh` 与 `install/image.lock`；
  「预制镜像」一节整体重写成唯一交付路径的三种拿法。
* `docs/pitfalls.md`：新增 **§七**（交付路线）四小节 —— 面板版本漂移与补丁门禁、
  手机直连 GitHub 大附件的间歇性超时、设备侧脚本不能带中文、`.ps1` 改完要补 BOM；
  并在 §一.6（面板反爬 UA 分界线）补上这次的复现与**逐步排除路径**
  （`uri_match` 匹配 → IP/域名检查对本机返回 False → 只剩 `is_spider()`）。
