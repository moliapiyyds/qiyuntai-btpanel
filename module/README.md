...
7. **面板 SSL 默认是关的**，别被 bt 的「自动申请 IP 证书」任务打开：装完面板后不到一小时，
   那个每小时的 `check_panel_ssl` 任务会自动给面板 IP 签自签证书并写
   `/www/server/panel/data/ssl.pl`，面板从此**只收 HTTPS** ——
   明文 `http://…` 连上就被 reset（不是 404），很容易误判成"面板没起来"。
   本模块部署时会把那个脚本空壳化并删掉 `ssl.pl`；万一它又回来了：
   ```sh
   rm -f /www/server/panel/data/ssl.pl && /etc/init.d/bt restart   # chroot 内
   ```
   想**主动用 HTTPS** 也行：面板设置里开 SSL 即可，这时访问地址要用 `https://`，
   模块自检与「执行」按钮打印的地址会自己跟着变成 https。
