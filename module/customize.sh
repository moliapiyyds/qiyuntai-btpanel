...
if [ -d "$ROOT/www/server/panel" ]; then
    ...
    echo "但需要先把 openEuler 铺好再重启："
    echo ""
    echo "  # 1) 铺 rootfs（支持 --url 在线下载，或先在电脑上下好再 --xz/--tar 推过来）"
    echo "  sh /sdcard/prepare-rootfs.sh --url <清华镜像的 tar.xz 地址>"
    echo ""
    echo "  # 2) 装面板 + 组件 + 打补丁（在仓库根目录执行）"
    echo "  sh install/qiyuntai-install.sh"
    echo ""
    echo "  # 3) 重启"
    echo "  reboot"
    echo ""
