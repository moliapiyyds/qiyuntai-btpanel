...
if [ -d "$ROOT/www/server/panel" ]; then
    ...
    echo "要手搓的话（文件已经推到 /sdcard/install 与 /sdcard/tools）："
    echo ""
    echo "  # 1) 铺 rootfs（--mirror 让它自己到镜像站挑文件；也可 --url 指定具体地址）"
    echo "  sh /sdcard/install/prepare-rootfs.sh --mirror"
    echo ""
    echo "  # 2) 装面板 + 组件 + 插件 + 补丁（all 会按顺序全做）"
    echo "  sh /sdcard/install/qiyuntai-install.sh all"
    echo ""
    echo "  # 3) 重启"
    echo "  reboot"
    echo ""
    echo "也可以一步到位（手机侧自举）： sh /sdcard/install/deploy.sh"
